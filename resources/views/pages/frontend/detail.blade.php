...
new #[Layout('layouts.frontend')] class extends Component {
    public $slug;
    public $quantity = 1;
    public $isDecraseButtonDisabled = false;

    #[Computed]
    public function product()
    {
        return app(GetProduct::class)->getProductBySlug($this->slug);
    }

    #[Computed]
    public function relatedProducts()
    {
        return app(GetProduct::class)->getRelatedProductsByCategoryId($this->product->category_id, $this->product->id);
    }

    public function disableDecreaseButton()
    {
        if ($this->quantity <= 1) {
            return $this->isDecraseButtonDisabled = true;
        }

        return $this->isDecraseButtonDisabled = false;
    }

    public function incraseQuantity()
    {
        $this->quantity++;
    ...
    {
        try {
            app(AddToCart::class)->handleAddToCart($this->product->id, $this->quantity);

            $this->dispatch('notify', message: 'Product added to cart successfully!');

        } catch (\Exception $e) {
            Log::error("error add to cart: " . $e->getMessage());
            Flux::toast('Failed to add product to cart.');
        }
    }
};
...
<div>
    ...
    <section class="bg-gray-100 py-8 px-4">
        <div class="container mx-auto">
            <ul class="breadcrumb">
                <li>
                    <a class="text-md md:text-lg" href="{{ route('home') }}" wire:navigate>Home</a>
                </li>
                <li>
                    <a class="text-md md:text-lg" href="#">{{ $this->product->category->name }}</a>
                </li>
                <li>
                    <a class="text-md md:text-lg" href="#" aria-label="current-page">{{ $this->product->name }}</a>
                </li>
            </ul>
        </div>
    </section>
    ...
    <section class="container mx-auto">
        <div class="flex flex-wrap my-4 md:my-12">
            <div class="flex-1">
                <div class="slider">
                    <div class="thumbnail">
                        @foreach ($this->product->galleries as $item)
                            <div class="px-2">
                                <div class="item {{ $loop->first ? 'selected' : '' }}"
                                    data-img="{{ asset('storage/' . $item->image) }}">
                                    <img src="{{ asset('storage/' . $item->image) }}" alt="front"
                    ...
                    <div class="preview">
                        <div class="item rounded-lg h-full overflow-hidden">
                            <img src="{{ $this->product->galleries()->exists() ? asset('storage/' . $this->product->galleries->first()->image) : 'data:image/gif;base64,R0lGODlhAQABAIAAAMLCwgAAACH5BAAAAAAALAAAAAABAAEAAAICRAEAOw==' }}"
                                alt="front" class="object-cover w-full h-full rounded-lg" />
                        </div>
                    </div>
                </div>
            </div>
            <div class="flex-1 px-4 md:py-4 md:px-4">
                <h2 class="text-2xl md:text-3xl font-semibold mb-1 md:md-2">{{ $this->product->name }}</h2>
                <div class="flex justify-start mb-4 md:mb-8">
                    <div class="text-3xl md:text-4xl font-bold">{{ formatRupiah($this->product->price) }}</div>
                </div>
                <div class="flex flex-wrap items-center mb-2 md:mb-4">
                    <p class="text-lg md:text-xl font-semibold text-gray-500">Quantity</p>
                    <div class="flex px-4">
                        <button type="button" class="font-semibold text-lg px-4 border border-gray-400"
                            {{ $this->disableDecreaseButton() ? 'disabled' : '' }}
                            wire:click='decreaseQuantity'>-</button>
                        <h6 class="font-semibold text-lg px-4 border border-gray-400">{{ $this->quantity }}</h6>
                        <button type="button" class="font-semibold text-lg px-4 border border-gray-400"
                            wire:click="incraseQuantity">+</button>
                    </div>
                </div>

                @if ($this->product->stock == 0)
                    <p class="text-md text-red-600 font-semibold">Sold out</p>
                @else
                    <p class="text-md text-red-600 font-semibold">Current stock
                ...
                <button type="button" wire:click='addToCart' {{ $this->product->stock == 0 ? 'disabled' : '' }}
                    class="transition-all duration-200 bg-pink-400 text-black focus:bg-black focus:text-pink-400 rounded-full px-8 py-3 mt-4 inline-flex"><svg
                        class="fill-current mr-3" width="26" height="24" viewBox="0 0 26 24" wire:navigate>
                        <path
                ...
                <h6 class="text-xl md:text-2xl font-semibold mb-2">About the product</h6>
                <p class="text-md lg:text-lg leading-7 mb-6 font-medium text-gray-600"> {!! $this->product->description !!} </p>
            </div>
        </div>
    </section>
    <!-- END: DETAILS -->
        ...
        <div class="container mx-auto">
            <div class="flex flex-start mb-4">
                <h3 class="text-2xl capitalize font-semibold">
                    Complete the look <br class="" />with our curated selection
                </h3>
            </div>
                    ...
                    <div class="px-3 flex-none" style="width: 320px">
                        <div class="rounded-xl p-4 pb-8 relative bg-white">
                            <div class="rounded-xl overflow-hidden card-shadow w-full h-36">
                                <img src="{{ $relatedProduct->galleries()->exists() ? asset('storage/' . $relatedProduct->galleries->first()->image) : 'data:image/gif;base64,R0lGODlhAQABAIAAAMLCwgAAACH5BAAAAAAALAAAAAABAAEAAAICRAEAOw==' }}"
                                    alt="" class="w-full h-full object-cover object-center" />
                            </div>
                            <h5 class="text-lg font-semibold mt-4">{{ $relatedProduct->name }}</h5>
                            <span class="">{{ formatRupiah($relatedProduct->price) }}</span>
                            <a href="{{ route('detail', $relatedProduct->slug) }}" class="stretched-link"
                                wire:navigate>
                                <!-- fake children -->
                            </a>
                        </div>
